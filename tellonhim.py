...
prefs = {"profile.managed_default_content_settings.images": 2, "profile.default_content_setting_values.notifications": 2, "profile.default_content_setting_values.media_stream_camera": 2, "profile.default_content_setting_values.media_stream_mic": 2, "profile.default_content_setting_values.plugins": 2, "profile.default_content_setting_values.popups": 2, "profile.default_content_setting_values.geolocation": 2, "profile.default_content_setting_values.auto_select_certificate": 2, "profile.default_content_setting_values.fullscreen": 2, "profile.default_content_setting_values.mouselock": 2, "profile.default_content_setting_values.mixed_script": 2, "profile.default_content_setting_values.peer_connection": 2, "profile.default_content_setting_values.media_stream": 2, "profile.default_content_setting_values.automatic_downloads": 2, "profile.default_content_setting_values.midi_sysex": 2, "profile.default_content_setting_values.push_messaging": 2, "profile.default_content_setting_values.ssl_cert_decisions": 2, "profile.default_content_setting_values.ppapi_broker": 2, "profile.default_content_setting_values.protectedContent": 2, "profile.default_content_setting_values.app_banner": 2, "profile.default_content_setting_values.site_engagement": 2, "profile.default_content_setting_values.durable_storage": 2}
...
def create_answer_tables(answers):
    tables = []
    for answer in answers:
        ...
        tables.append(Panel.fit(table, border_style="bold blue"))
    return tables
...
def check(user):
    ...
    info = {
        ...
        "Total Followers": str(int(data.get("followerCount")) + int(data.get("anonymousFollowerCount"))),
        ...
        "Is Able To Comment": str(data.get("isAbleToComment", "N/A")),
    ...
    console = Console()
    table = Table(title=f"User Information (@{user})", header_style="bold magenta")
    ...
    answer_tables = create_answer_tables(answers)

    # Organize tables into rows of three
    rows = []
    num_tables_per_row = 3
    for i in range(0, len(answer_tables), num_tables_per_row):
        row = answer_tables[i:i + num_tables_per_row]
        row_layout = Layout()
        row_panels = [Layout(panel) for panel in row]
        row_layout.split_row(*row_panels)
        rows.append(row_layout)

    # Combine all answer table rows into a single panel
    combined_rows = Group(*rows)
    combined_answers_panel = Panel(combined_rows, title=f"Tells (@{user})")

    # Print the combined answers panel
    console.print(Align.center(combined_answers_panel))
    console.print(table, justify="center")
text = "Made by Chico (@sirchico), you are not allowed to sell this tool."
